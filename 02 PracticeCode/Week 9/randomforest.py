...
class RandomForest:
    ...
    def classify(self, test):
        ...
        for tree in self.trees:
            ...
            result = tree.classify(test)  # result = 'republican' or 'democrat'
            # ???????????????
...
if __name__ == "__main__":
    ...
    testingRecords = []
    cnt = 0
    for row in reader:  # row가 총 435개
        record = Record(row)
        cnt = cnt + 1
        if cnt < 300:  # 1~299 : 299개
            trainingRecords.append(record)
        else:  # 300~435 : 136개
            testingRecords.append(record)
    print("len(trainingRecords): ", len(trainingRecords))
    print("len(testingRecords): ", len(testingRecords))

    forest = RandomForest(trainingRecords, 200, 3, 3)  # (records, numSubSample, numTrees, maxDepth):
